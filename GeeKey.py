...
class DockBarIcon(wx.adv.TaskBarIcon):
    ...
    def CreatePopupMenu(self):
        ...
        self.configure_menu =  self.menu.Append(wx.ID_ANY,lt('Configure') )
        self.Bind(wx.EVT_MENU, self.frame.OnConfigure, self.configure_menu )

        self.loadfrom_menu = self.menu.Append(wx.ID_ANY, lt('Load from') )
        self.Bind(wx.EVT_MENU, self.frame.OnLoadFrom, self.loadfrom_menu  )

        self.saveas_menu =  self.menu.Append(wx.ID_ANY,lt('Save as') )
        self.Bind(wx.EVT_MENU, self.frame.OnSaveAs,self.saveas_menu)

        self.tutorial_menu = self.menu.Append(wx.ID_ANY,lt('Tutorial') )
        self.Bind(wx.EVT_MENU, self.frame.OnTutorial, self.tutorial_menu )

        if self.frame.IsShown():
            self.show_menu = self.menu.Append(wx.ID_ANY,lt('Hide panel') )
            self.Bind(wx.EVT_MENU, lambda evt: self.frame.Hide(), self.show_menu )
        else:
            self.show_menu = self.menu.Append(wx.ID_ANY,lt('Show panel') )
            self.Bind(wx.EVT_MENU, lambda evt: self.frame.RaiseShow(), self.show_menu )
            pass

        if self.frame.vim.vim_on:
            self.vim_menu = self.menu.Append(wx.ID_ANY,lt('Disable Vim-Mode') )
        else:
            self.vim_menu = self.menu.Append(wx.ID_ANY,lt('Enable Vim-Mode') )
            pass
        self.Bind(wx.EVT_MENU, lambda evt: self.frame.vim.state_switch(), self.vim_menu )

        
        self.exit_menu = self.menu.Append(wx.ID_ANY,lt('Exit') )
...
class GeeKeyFrame(wx.Frame):
    def __init__(self,parent,ID):
        self.initing = True
        self.dockBarIcon = DockBarIcon(self)
        #############################
        ...
        self.data_dir = os.path.join( self.home_dir, 'dat' )
        self.plugin_dir = os.path.join( self.home_dir, 'plugin' )

        self.layout_file_name = 'default.ini'
        self.version_file = os.path.join( self.data_dir, 'v'+INFO['Version'] )
        ...
        ##### UI init
        self.size = (985, 585)
        style = wx.NO_BORDER| wx.CLOSE_BOX| wx.SYSTEM_MENU;
        ...
        #######################################################
        ##### binding events
        # self.Bind( wx.EVT_SHOW, self.OnIconize )
        self.Bind( wx.EVT_ICONIZE, self.OnIconize )
        self.Bind( wx.EVT_CLOSE, self.OnIconize )
        #self.Bind( wx.EVT_MINIMIZE, self.OnIconize )
        ...
        ####
        self.initing = False
        #WxCallLater(0.5,self.RaiseShow )
    ...
    def OnExit(self,event):
        ...
        self.dockBarIcon.RemoveIcon()
        self.dockBarIcon.Destroy()
    ...
    def makePanel(self):
        ...
        self.comboPanel = wx.Panel(self.mainPanel,pos=(0,0),size=(985,411) )
        self.comboPanel.SetBackgroundColour('#4A708B')

        self.keyboardPanel = wx.Panel(self.mainPanel,pos=(0,0),size=(985,411 ) )
        self.keyboardPanel.SetBackgroundColour('#4A708B')


        ##############################################################
        self.bottomPanel = wx.Panel(self.mainPanel,pos=(0,411),size=(985,220) )
        ...
        for i in range(4):
            bt = wx.Button(
                self.keyboardPanel, pos = ( 58+bx+i*width,86+by),
                size=(width,height),
                style=0|wx.BU_LEFT|wx.BORDER_NONE
            )
            bt.SetBackgroundColour( GetColorMap(  self.key_types[i]+'_key') )
            bt.SetToolTip( lt("_"+self.key_types[i]+"_color_info") )
            pass

        self.geekey_set_button = wx.Button(
            self.keyboardPanel, label= 'GK',
            pos =(26+100+bx,18+by),size=(47,28),
            style=0|wx.BU_LEFT|wx.BORDER_NONE)
        ...
        self.keyboard_clear_button = wx.Button(
            self.keyboardPanel, label= 'Clear',
            pos =(26+100+bx,49+by),size=(47,28),
            style=0|wx.BU_LEFT|wx.BORDER_NONE )
        ...
        self.makeGeeKeyButton('esc',(22+35+bx,18+by),KSize,keytype='menu')
        ...
        by = - 15;
        self.makeGeeKeyButton('`',(22+35+bx,109+by),KSize,keytype='text')
        ...
        self.makeGeeKeyButton("backspace",(841+35+bx,109+by),size=(KSize[0]+65,KSize[1]),keytype='edit')

        self.makeGeeKeyButton('tab',(22+35+bx,170+by),size=(KSize[0]+32,KSize[1]),keytype='menu')
        ...
        self.makeGeeKeyButton('\\',(873+35+bx,170+by),size=(KSize[0]+33,KSize[1]),keytype='edit')

        self.makeGeeKeyButton('caps lock',(22+35+bx,231+by),(KSize[0]+46,KSize[1]),keytype='menu')
        ...
        self.makeGeeKeyButton("return",(824+35+bx,231+by),size=(KSize[0]+82,KSize[1]),keytype='edit')
        
        self.makeGeeKeyButton("left shift",(22+35+bx,292+by),size=(KSize[0]+78,KSize[1]),keytype='menu')
        ...
        self.makeGeeKeyButton("right shift",(793+35+bx,292+by),size=(KSize[0]+113,KSize[1]),keytype='menu')

        ####
        self.makeGeeKeyButton('left ctrl',(22+35+bx,353+by),(KSize[0]+31,KSize[1]),keytype='menu')
        self.makeGeeKeyButton('left cmd',(116+35+bx,353+by),(KSize[0]+16,KSize[1]),keytype='menu')
        self.makeGeeKeyButton('left alt',(195+35+bx,353+by),(KSize[0]+16,KSize[1]),keytype='menu')
        self.makeGeeKeyButton('space',(274+35+bx,353+by),size=(KSize[0]+300,KSize[1]),keytype='plain')
        
        self.makeGeeKeyButton('right alt',(637+35+bx,353+by),(KSize[0]+16,KSize[1]),keytype='menu')
        self.makeGeeKeyButton('right cmd',(716+35+bx,353+by),(KSize[0]+16,KSize[1]),keytype='menu')
        self.makeGeeKeyButton('menu',(795+35+bx,353+by),(KSize[0]+16,KSize[1]),keytype='menu')
        self.makeGeeKeyButton('right ctrl',(874+35+bx,353+by),(KSize[0]+32,KSize[1]),keytype='menu')

        self.keyboardPanel.Refresh()
        pass
    ...
    def LoadConfig(self,filename='default.ini',changeMenuKey=False):
        ...
        ### configfile settings 
        if not os.path.exists( self.home_dir ): os.mkdir( self.home_dir )
        if not os.path.exists( self.plugin_dir ): os.mkdir( self.plugin_dir )
        ...
        if not os.path.exists( self.version_file ) and os.path.exists( self.configFilename):
            WxCallLater(5, log.log, lt('default.ini has been updated for new version. Original default.ini has been renamed to default.ini.bak') )
            #touch version file, and rename configFilename
            shutil.move(self.configFilename,self.configFilename + '.bak')
            open(self.version_file, 'a').close()
    ...
    def getLayoutList(self):
        ...
        for item in lists:
            if item.lower()[-4:] == '.ini':
                ...
                pass
            pass
        #log.log("get layout list finished")
        pass
    ...
    def makeGeeKeyButton(self,name,pos,size=KSize,keytype='plain'):
        dx = 5;
        ...
        if keytype in ('macro','text','edit','function'): ### geekey keys
            ...
            ###########################
            self.key_buttons[name] = wx.Button(self.keyboardPanel, label = GetKeyText(name), pos = pos, size = size, style = 0|wx.BORDER_NONE )
            ...
            ##########################################
            self.key_combos[name] = wx.ComboBox( self.comboPanel,value='',pos=cpos,size=(100,1),choices = EditKeys ) 
            ...
            self.key_buttons[name] = wx.Button(
                self.keyboardPanel, label = GetKeyText(name), pos = pos, size = size,
                style=0|wx.BU_LEFT|wx.BORDER_NONE
            )
    ...
    def onGeeKeyComboSelected(self,evt,name):  
    ...
    def onKeyboardClear(self,evt=None):
        self.stateVarInit()
        for (key,code) in list( Key2Code.items() ):
            if key and code >=0 and code < 999: KeyRelease(key )
    ...
    def Replay(self,ratio,sequence):
        #raise Exception('why replay')
        #print('replay with ',ratio,sequence)
    ...
    def _Replay(self,ratio,sequence):
        ...
        if len( oper ) <= 2: #compact oper for keyboard  :_x~0.1  :x~  :x  :_x
            ...
            KeySend( key = key, keytype = keytype, flags = ReplayMask )
            ...
            if oper[0] == 'mma':#:mma~x~y~0.1
                ...
                pos = None
                if oper[1] or oper[2]: pos = (toFloat(oper[1]),toFloat(oper[2]) )
                #print('click or wheel at',pos)
                if  len(oper) <= 4:
                    #print('click at',pos)
                    geeMouse.buttonEvent( rmouseEventTypeMap[ oper[0] ], pos, )
                    if len(oper) == 4 and oper[3]: delay = toFloat( oper[3] )
                else:
                    geeMouse.buttonEvent( rmouseEventTypeMap[ oper[0] ], pos, int(oper[3])  )
                    #print('wheel at',pos)
    ...
    def OnKeyboardEvent(self, Key, EvtType, EvtScanCode ): 
        try:
            #### alias asignment
            SendKey = lambda key,repeated=1: KeySend(key,KeyCode,EvtType,repeated)
            IsKeyDown = EvtType == 'key down'
            IsKeyUp =  EvtType == 'key up'
            ...
            if self.state_is_recording and not EvtScanCode in (ScanCodeRevised,ScanCodeReplay):
            ...
            if  not Key in self.GeeKeyKeys:
                ...
                ###################################################
                #alt key state
                if ( (Key == 'left cmd' or Key == 'right cmd') ):
                    self.state_on_cmd = (True if IsKeyDown else  False)
                    return True
                ###################################################
    ...
    def OnMouseEvent(self,EvtType,EvtPosition,EvtWheel):
        try:
            if ( EvtType == 'mouse move' ): return True
            #######################################
            ### redording sequence
            if self.state_is_recording:
                if not self.recording_last_time:
                    self.recording_last_time = time.time()
                res = (
                    mouseEventTypeMap[EvtType], EvtPosition[0], EvtPosition[1],
                    "%.2f"%(time.time() - self.recording_last_time )
                ) if ( EvtType != kCGEventScrollWheel )  else (
                    mouseEventTypeMap[EvtType], EvtPosition[0],EvtPosition[1],
                    EvtWheel, "%.2f"%(time.time() - self.recording_last_time)
                )
                self.recording_last_time = time.time()
                #log.log res
                self.recording_sequence.append( res )
            ...
            if self.getConfig('doubleclickfix') == 'True':
                #log.log( EvtType ,time.time() )
                if EvtType == kCGEventLeftMouseUp:
                    if time.time() - self.last_left_up_time < 0.05:
                        self.last_left_up_time = time.time()
                        return False
                    self.last_left_up_time = time.time()
                if EvtType == kCGEventLeftMouseDown:
                    if time.time() - self.last_left_up_time < 0.05:
                        self.last_left_up_time = time.time()
                        return False
                    pass
                if EvtType == kCGEventRightMouseUp:
                    if time.time() - self.last_right_up_time < 0.05:
                        self.last_right_up_time = time.time()
                        return False
                    self.last_right_up_time = time.time()
                if EvtType == kCGEventRightMouseDown:
                    if time.time() - self.last_right_up_time < 0.05:
                        self.last_right_up_time = time.time()
                        return False
                    pass
