...
class AutoComplete(wx.Frame):
    ...
    def UpdateWord(self,word,freq = 1):
        ...
        if res:
            ...
            #self.word_list.insert([word.lower(),word,freq] ) #sortedcontainers updated
    ...
    def SwitchState(self,state=None):
        if not state:
            ...
            if not self.auto_complete_on: #turn off
                self.StateReset()
                self.geekey.taskBarIcon.ShowBalloon( lt('_autocomplete'),lt('_autocomplete_off'))
                pass
            else: #turn on
                self.geekey.taskBarIcon.ShowBalloon( lt('_autocomplete'),lt('_autocomplete_on'))
                pass
            #log.log("get out geekey + tab")
            pass
        else:
            if state == 'True' or state == 'on': self.auto_complete_on = True
            if state == 'False' or state == 'off': self.auto_complete_on = False
            pass
        pass
